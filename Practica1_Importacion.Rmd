...
Bread <- read.csv(filename)
```
```{r}
Bread$Item<-as.factor(Bread$Item)
Bread$Date<-as.POSIXlt(Bread$Date,tryFormats ="%Y-%m-%d" )
Bread$Time<-as_hms(Bread$Time)
save(Bread, file = "./data/BreadBasket_DMS.RData")
```
...
JSON<- read_json(filename, simplifyVector = T)
JSONmal<- read_json(filename, simplifyVector = F)
save(JSON,file = "./data/FileCodificado.RData")
```

Gracias a `simplifyVector` puedo crear el dataframe por atributos en vez de tenerlo todo en una lista terrible donde cada elemento es un individuo desglosado por atributos.
